...
class LooperEngine {
  ...
  private masterLoopLength: number = 0; // Length of track 1 determines master loop length
  private sourceNodes: Map<string, AudioBufferSourceNode> = new Map();
  private gainNodes: Map<string, GainNode> = new Map();
  private loopStartTime: number = 0;
  private loopTimer: number | null = null;
  private historyStack: Map<string, AudioBuffer[]> = new Map(); // For undo/redo
  ...
  /**
   * Start recording or overdubbing on active track
   */
  public async startRecord() {
    ...
    try {
      const { stream } = await audioEngine.startMicrophone();
      ...
      this.mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          this.saveTrackBuffer(this.activeTrackIndex, audioBuffer);
        } catch (err) {
          console.error("Looper decode error:", err);
        }
      };
      ...
      // If other tracks exist and we're not playing yet, start playback synced
      ...
      alert("Microphone permission required for guitar looper recording.");
    }
  }
  ...
  private saveTrackBuffer(trackIdx: number, newBuffer: AudioBuffer) {
    ...
    // If this is track 1 (base loop), set master length
  ...
  public clearTrack(trackIdx: number | string) {
    ...
    // If track 0 was cleared, reset master loop length if all empty
