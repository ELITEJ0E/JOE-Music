...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <nav className="md:hidden fixed bottom-0 inset-x-0 bg-[#0d0f12]/95 backdrop-blur-xl border-t border-[#191d24] px-4 py-2 flex items-center justify-around z-40">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 py-1 px-3 rounded-xl transition-all ${
              isActive ? "text-[#00FF66] font-bold" : "text-zinc-500 hover:text-zinc-300"
            }`}
          >
